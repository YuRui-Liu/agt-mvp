...
4. 脚本会在本机打开页面，并在终端打印本地 URL。不得在聊天或公开日志中复述、粘贴或直接转发包含 token 的完整 URL。若浏览器未自动打开，只提示用户查看自己的终端；仅当用户明确请求、确认处于私密上下文时，才协助其从终端定位地址，仍不得直接转发 token。
...
缺少 Python 3、必要模板、可执行的 `agentsview`，或本地服务启动失败时停止。同步失败默认停止；只有用户已理解可能使用旧数据并明确同意时，才可用 `AVSCORE_SKIP_SYNC=1` 重启。没有可选会话时服务仍会正常启动并展示空状态页，供用户按页面提示排障，不应将其误报为启动失败。
